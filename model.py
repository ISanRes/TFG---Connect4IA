...
class Gen_Model():
	...
	def read(self, path):
		return load_model(path + 'bestVersion.h5',
...
class Residual_CNN(Gen_Model):
	...
	def _build_model(self):
		...
		model.compile(loss={'value_head': 'mean_squared_error', 'policy_head': softmax_cross_entropy_with_logits},
					  optimizer=SGD(lr=self.learning_rate, momentum=settings.MOMENTUM),
					  loss_weights={'value_head': 0.5, 'policy_head': 0.5}
					  )

		return model

	def convertToModelInput(self, board):

		inputToModel = np.zeros((2,6,7))
		board = board.convertIntoTable()
		for col in range(board.WIDTH):
			for row in range(board.HEIGHT):
				if board[col][row] == -1:
					inputToModel[0][col][row] = 0
					inputToModel[1][col][row] = 0
